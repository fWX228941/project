...
public class CharacterParserUtil {
    ...
    public String getSelling(String chs) {
        Log.d("CitySelecterActivity", "02");
        String key, value;
        buffer = new StringBuilder();
        Log.d("CitySelecterActivity", "buffer"+buffer);
        for (int i = 0; i < chs.length(); i++) {
            key = chs.substring(i, i + 1);
            Log.d("CitySelecterActivity", key);
            if (key.getBytes().length >= 2) {
                value = (String) convert(key);
                Log.d("CitySelecterActivity", value);
                if (value == null) {
                    value = "unknown";
                }
            } else {
            ...
            buffer.append(value);
            Log.d("CitySelecterActivity",  buffer.toString());
        }
