...
/**
 * @author fumin
 * 参考设计文档：https://github.com/fWX228941/progress-activity  【三个视图定制化，这个太值得学习了】本质上就是一个相对布局，编程步骤参考文档
 * 这个定制化，到时再来修改下样式，统一风格
 * 1.错误视图 【联网失败】
 * 2.空视图 【没有更多数据】
 * 3.进度条：更加定制化
 *
...
public class ProgressActivity extends RelativeLayout {
    ...
    /**
     * Hide content and show the progress bar  进度条，到时修改
     */
    ...
    public void showEmpty( String emptyTextContent) {
        switchState(EMPTY, null, null, emptyTextContent, null, null, Collections.<Integer>emptyList());
    }
    ...
    public void showError(String errorTextContent, String errorButtonText, OnClickListener onClickListener) {
        switchState(ERROR, null, null, errorTextContent, errorButtonText, onClickListener, Collections.<Integer>emptyList());
    }
    public void showError(String errorTextContent, OnClickListener onClickListener) {
        switchState(ERROR, null, null, errorTextContent, null, onClickListener, Collections.<Integer>emptyList());
    }
    /**
     * Show error view with a button when something goes wrong and prompting the user to try again
     *
    ...
    /*在这里定制加载加载视图，结合loadingView 和文字效果*/
...
/*            emptyStateImageView.getLayoutParams().width = emptyStateImageWidth;
            emptyStateImageView.getLayoutParams().height = emptyStateImageHeight;款高度重新绘制了
...
/*            layoutParams = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
            ...
            this.addView(emptyStateRelativeLayout, new LayoutParams(ViewGroup
                    .LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        } else {
            emptyStateRelativeLayout.setVisibility(VISIBLE);
        }
    }
    /*在这里定制各种异常情况导致没有数据需要重新加载的情况*/
