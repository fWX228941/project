 ...
 * @创建日期：2016/05/23
 * @邮箱：466566941@qq.com
 * @当前文件描述：自定义刷新布局，延伸SwipeRefreshLayout源码。【可能这个又用不到了】
 * 参考文档：http://blog.csdn.net/nupt123456789/article/details/47980689  https://github.com/nuptboyzhb/SuperSwipeRefreshLayout
 * 功能点：
 * 01.非侵入式，对原来的listView，recyclerView没有任何影响，用法和swipeRefreshLayout，支持任何可滚动的控件
 * 02.调用setHeaderView/footerView可自定义下拉刷新头部的View的样式
 * 03.默认setTargetScrollWithLayout(true) 嵌入的滚动视图随手指的滚动而滚动
 * 04.自定义动漫效果：根据下拉过程中distance的值，onPulLDistance(int distance) onPullEnable(boolean enable)
 * 05.支持下拉刷新，上拉加载更多的回调方法
 *
 * 思路：
 * 01.自定义一个ViewGroup ，往其中添加headerView 和scrollView，然后在onMeasure中确定他们的大小，在onLayout中确定他们位置
 * 02.当listView滑动到最上方或者最下方的时候，ViewGroup拦截用户事件并自己处理onTouchEvent事件，其他正常情况下，交由ListView自行处理onTouchEvent事件
 *
 */
/*忽略这个警告，有可能会与点击事件发生冲突*/
...
public class SuperSwipeRefreshLayout extends ViewGroup {
    ...
    //2.SuperSwipeRefreshLayout内的目标View，比如RecyclerView,ListView,ScrollView,GridView，可滚动的View
    private View mTarget;
    /*3.两个回调分别是上拉加载更多和下拉刷新*/
    ...
    /**
     * 1.孩子节点绘制的顺序，getChildDrawingOrder：第i次应该绘制哪一个childView
     * 2.因为HeaderView和FooterView刚开始是不显示的，分别隐藏在界面的上方和下方，因此需要将它们两个的绘制顺序调整到最后
     *
     *
     *
     */
    ...
    /*onLayout决定了View的位置，当拦截到用户滑动事件后，可以根据滑动的距离，动态更新View的位置，所以headerView 和footView的layout 跟滑动距离有关系*/
    ...
    /**
     * 01.下拉刷新核心方法
     * 02。判断目标View是否滑动到顶部-还能否继续滑动，滚动view的判断逻辑不尽相同
     *
     *
     * @return
     */
    ...
    /**
     * 01.主要判断是否应该拦截子View的事件，如果拦截，则交给自己的OnTouchEvent处理，否者，交给子View处理
     * 02.onInterceptTouchEvent 决定了事件到底交给谁处理
     * 03.
     * 当返回return true 时，表示ViewGroup自己来处理onTouchEvent事件，子View接收不到onTouchEvent事件
     * 当返回return false时，表示ViewGroup不拦截事件。，直接交给子View来处理
     * 04.我们不定需要在点击时判断是否拦截事件，还需要在滑动过程中判断是否需要拦截事件，比如滑动的距离过小，则不需要拦截
     *
     */
    @SuppressLint("LongLogTag")
    @Override
    ...
    /*
        通过在superLayout中，onTouchEvent 只有当onInterceptTouchEvent返回true来执行，它跟根据下拉或者上拉的距离，动态的修改headerView和
    * FootView的位置，通过该offSetTopAndBottom 刷新onLayout方法
    *
    * */
    @Override
