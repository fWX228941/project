...
import static com.baidu.location.b.k.cz;

/**
 * 创建人 : fumin
 * 创建时间 : 2016-10-08上午11:02:34<br>
 * 版本 : [v1.0]<br>
...
//	private IWXAPI api;
	 ...
	 *            商品金额费用, 单位是分
	 */
	public WeChatPayService(Context context, String out_trade_no,
	...
	public void pay() {
		...
		if (!BaseApplication.api.isWXAppInstalled()) {
			ToastUtil.showHintMessage("您还未安装微信客户端,支付失败");
			return;
		}

		boolean isWeChat = BaseApplication.api.getWXAppSupportAPI() >= Build.PAY_SUPPORTED_SDK_INT;
		Log.d(TAG, "isWeChat:" + isWeChat);
		if (isWeChat) {
			new GetPrepayIdTask().execute();
		}else {
			ToastUtil.showHintMessage("当前微信版本暂时不支持支付，请升级微信版本，再来支付");
		}
	}
	...
	private void sendPayReq(String prepayId) {
		...
		req.timeStamp = String.valueOf(genTimeStamp()); // 这个地方时间戳会不会有问题，海德待定HeaderUtil.getHeadindex2(NTPTime.getInstance().getCurrentTime());
