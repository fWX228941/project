...
/**
 * Created by fumin on 16/8/11.
 * 自定义随机验证码
 * 参考文档：https://github.com/myloften/VerificationCodeSample
 * http://blog.csdn.net/loften_93663469/article/details/51367979
 */
public class ValidationCodeView extends View {
    ...
    private void getAttrValues(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.ValidationCodeView);
        mCodeCount = typedArray.getInteger(R.styleable.ValidationCodeView_CodeCount, 4);
        mPointNumber = typedArray.getInteger(R.styleable.ValidationCodeView_PointNumber, 100);
        mLineNumber = typedArray.getInteger(R.styleable.ValidationCodeView_LineNumber, 2);
        mTextSize = typedArray.getDimension(R.styleable.ValidationCodeView_CodeTextSize, 20);
        mBackGround = typedArray.getColor(R.styleable.ValidationCodeView_BackGround,Color.WHITE);
        typedArray.recycle();
    }
