 ...
 * @当前文件描述：充值明细界面
 */
public class ChargeCapitalActivity extends BaseActivity  implements BaseQuickAdapter.RequestLoadMoreListener,SwipeRefreshLayout.OnRefreshListener {
    private static final int LOGIN_TO_CHARGECAPITAL = 3;
...
//    List<ChargeBean> listData = new ArrayList<ChargeBean>();
    ...
    private void loginForToken() {
        ...
        IntentUtil.gotoActivityForResult(ChargeCapitalActivity.this, LoginActivity.class, LOGIN_TO_CHARGECAPITAL);//跳转到登录界面
    }
    ...
    private void handleLogic() {
        ...
        if (isLogin) {
            ...
            progressActivity.showEmpty("亲，暂时还没有登录，请先到用户设置的登录界面登录，然后才能查看我的充值明细！");
        }
    ...
    /*01.循环视图设置布局管理器*/
        mSuperRecyclerViewImg.setLayoutManager(new LinearLayoutManager(ChargeCapitalActivity.this));
        /*03.设置下拉刷新的监听器*/
    ...
    private void getDataFromServer(final int currentPage,final boolean isRefreshOrFirstLoading) {
        ...
        this.appAction.getChargeList(currentPage, PAGESIZE, new ActionCallbackTripleListener<List<ChargeBean>,
                Integer, Integer>() {


            @Override
            public void onSuccess(List<ChargeBean> data, Integer pageIndex, Integer total) {
                if (progressDialog.isShowing()){
                    progressDialog.cancel();
                }
                ...
                if (totalCount == 0) {
                    progressActivity.showEmpty("充值为空");
                } else {
                        ...
                        if (!isRefreshOrFirstLoading) {
                            if (isListToBottom) {
                                ...
                                if (notLoadingView == null) {
                                    inflater = LayoutInflater.from(ChargeCapitalActivity.this);
    ...
    private void initTittleBar() {
        ...
        titleBar.addAction(new TitleBar.TextAction("账户余额") {
            @Override
            public void performAction(View v) {
                // 跳转到充值明细界面
                IntentUtil.gotoActivityWithoutData(ChargeCapitalActivity.this, AccountBalanceActivity.class, false);
            }
        });
        ...
        titleBar.setLeftClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                finish();
                ChargeCapitalActivity.this.overridePendingTransition(R.anim.exit_enter, R.anim.exit_exi);
            }
        });
        titleBar.setTitle("充值明细");
    ...
    //LOGIN_TO_CHARGECAPITAL

    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        super.onActivityResult(requestCode, resultCode, data);
        switch (requestCode) {
            case LOGIN_TO_CHARGECAPITAL:
